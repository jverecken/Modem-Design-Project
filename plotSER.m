...
semilogy(SNR(index),SER_awgn_TH(index),'-r','linewidth',1.5,'displayname','awgn'); hold on;
semilogy(SNR(index),SER_rayleigh_TH(index),'-k','linewidth',1.5,'displayname','rayleigh');
semilogy(SNR(index),SER_rayleigh_NC_ZF(index),'.-','Color',[0 1 1],'linewidth',1.5,'markersize',15,'displayname','not coded');
semilogy(SNR(index),SER_rayleigh_C_ZF(index),'.-','Color',[0 1 0],'linewidth',1.5,'markersize',15,'displayname','soft - known - coded');
semilogy(SNR(index),SER_rayleigh_C_ZF_NOKNOWLEDGE(index),'.:','Color',colors(1,:),'linewidth',1.5,'markersize',15,'displayname','soft - not known - coded');
semilogy(SNR(index),SER_rayleigh_C_ZF_HARD(index),'.:','Color',colors(2,:),'linewidth',1.5,'markersize',15,'displayname','hard - known - coded');
semilogy(SNR(index),SER_rayleigh_C_ZF_HARD_NOKNOWLEDGE(index),'.:','Color',colors(3,:),'linewidth',1.5,'markersize',15,'displayname','hard - not known - coded');
grid on;
%ylim([1e-6 1]);
xlabel('Es/N0 [dB]');
ylabel('SER');
